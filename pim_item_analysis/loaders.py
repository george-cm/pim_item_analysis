...
def add_doc_analysis_date_pair_loader(
    db_file: str | Path,
    name: str,
    ...
    with db_create_connection(db_file) as conn:
        ...
        db_create_table(
            conn,
            "doc_analyses_date_pairs",
            columns=columns,
            unique_index_columns=["name", "pim_export_date", "doc_request_date"],
        )

        column_str: str = ", ".join([f'"{k}"' for k in columns[1:].keys()])
        ...
        """
        cursor: sqlite3.Cursor = conn.cursor()
        cursor.execute(sql, (name, doc_request_date, pim_export_date))
